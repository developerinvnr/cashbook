				...
				<tr>
					<td valign="top">
						...
						if(isset($_REQUEST['pg']) && $_REQUEST['pg']>1){
							$amount = 0;
							$sql=mysql_db_query(DATABASE2,"SELECT exps_amt FROM tblcash1 INNER JOIN tblcash2 ON tblcash1.vouch_id = tblcash2.vouch_id WHERE location_id=".$lid." AND exps_id=".$eid." AND vouch_date BETWEEN '".date("Y-m-d",$dt1)."' AND '".date("Y-m-d",$dt2)."' ORDER BY vouch_date,vouch_no,seq_no LIMIT 0,".$start) or die(mysql_error());
							...
							<td>&nbsp;</td>';
							if($opBal<0){
								echo '<td align="Right">'.number_format(-1*$opBal,2,".","").' Dr</td>';
							} else {
								echo '<td align="Right">'.number_format($opBal,2,".","").' Cr</td>';
							}
							echo '<td>&nbsp;</td></tr>';
...
/***************************************111 O **************/
							...
							$sql=mysql_db_query(DATABASE2,"SELECT Sum(exps_amt) AS amount FROM tblcash1 INNER JOIN tblcash2 ON tblcash1.vouch_id = tblcash2.vouch_id WHERE location_id=".$lid." AND exps_id=".$eid." AND vouch_date<'".date("Y-m-d",$dt1)."'") or die(mysql_error());
							$res=mysql_fetch_assoc($sql);
							if($res['amount']<0)
								$opBal += -1*$res['amount'];
							else
...
if($_REQUEST['eid']==61 OR $_REQUEST['eid']==62)
{
						if($opBal<0){
							...
							echo '<td>&nbsp;</td>';
							echo '<td align="Right"><b>'.number_format($opBal,2,".","").'</b></td>';
						}
						echo '<td>&nbsp;</td>';
						echo '<td>&nbsp;</td></tr>';
}
